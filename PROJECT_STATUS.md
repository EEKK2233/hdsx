...
## 2026-07-14 协作插件与 Markdown 独立预览更新

- 新增根目录 `plugins/` 协作扩展机制：清单白名单加载、统一 API 前缀、按角色动态导航、iframe 静态页面宿主和示例插件。
- 新插件主要在 `plugins/<id>/` 内开发；核心代码只保留一次性加载入口。插件是受信任 Python 代码而非安全沙箱，生产环境必须审查后通过 `ENABLED_PLUGINS` 启用。
- 新增 `plugins.md`，说明目录、清单、后端接口、独立页面、权限、数据库迁移和测试流程。
