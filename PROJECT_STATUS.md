...
- 根据协作方式澄清，撤下运行时可选插件、动态导航和 iframe 宿主，改为根目录 `p/<模块>/` 源码组合机制；协作者功能通过少量固定 import 直接集成到现有业务页面。
- 网页爬虫实现已真实移动到 `p/course_web_crawler`，知识库 `WebImportService` 通过稳定适配层调用它；从项目根目录或 `backend` 目录启动均可加载，权限、事务、确认入库、RAG 索引仍由核心项目控制。
