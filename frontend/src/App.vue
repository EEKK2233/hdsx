...
const allNav:{to:string;parentTo?:string;label:string;roles:string[]}[]=[
...
const nav=computed(()=>allNav.filter(n=>auth.user&&n.roles.includes(auth.user.role)).map(n=>({...n,to:auth.user?.role==='parent'&&n.parentTo?n.parentTo:n.to})))
