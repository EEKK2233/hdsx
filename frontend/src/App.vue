...
<template><div class="shell"><aside v-if="!isPublic"><div class="brand">Edu Agent<small>{{labels[auth.user?.role||'']}}工作台</small></div><router-link v-for="n in nav" :to="n.to" :key="n.to">{{n.label}}</router-link></aside><main><header v-if="!isPublic" class="topbar"><nav class="breadcrumb" aria-label="当前位置"><router-link to="/">工作台</router-link><span>›</span><strong>{{currentLabel}}</strong></nav><div class="account-actions"><span>{{auth.user?.display_name}} · {{labels[auth.user?.role||'']}}</span><button class="logout-button" @click="auth.logout">退出登录</button></div></header><router-view v-slot="{Component}"><keep-alive :max="16"><component :is="Component"/></keep-alive></router-view></main></div></template>
