...
<template><div class="shell"><aside v-if="$route.path!='/login'"><div class="brand">Edu Agent<small>{{labels[auth.user?.role||'']}}工作台</small></div><router-link v-for="n in nav" :to="n.to" :key="n.to">{{n.label}}</router-link><button class="ghost" @click="auth.logout">退出登录</button></aside><main><header v-if="$route.path!='/login'"><span>AI 教育智能体</span><span>{{auth.user?.display_name}} · {{labels[auth.user?.role||'']}}</span></header><router-view/></main></div></template>
