...
const router=useRouter(),items=ref<any[]>([]),message=ref('')
async function load(){items.value=((await api.get('/lesson-resources')).data as any[]).filter(x=>x.is_saved)}
async function preview(item:any){await router.push({path:`/lesson/preview/${item.id}`,query:{from:'favorites',title:item.content?.title||item.title||'备课文件预览'}})}
...
async function remove(item:any){if(!confirm(`确定删除《${item.title}》吗？`))return;await api.delete(`/lesson-resources/${item.id}`);await load()}
